...
| BUG-027 | shipped-v0.41.2 | medium | AI Providers settings: image-routing shows empty, vector-indexing shows no data | `_renderImage` filtered on `p.is_ai_assist` (undefined) instead of `p.use_for_ai_assist` (int 0/1 from API) → image routing always showed "Not configured". `_renderVector` read `prefs.vector_indexer_url` which never existed → endpoint and status always empty. Fixed by: correcting field name in `_renderImage`; adding `GET /api/admin/vector-status` endpoint returning `{configured, host, collection, reachable}`; boot script fetches and passes `vectorStatus` to component; `_renderVector` displays live env-var config + reachability. |
