/* Boot script for the new-UX Pipeline Files page.
 * Fetches /api/me, mounts chrome + MFPipelineFiles.
...
(function () {
  ...
  var pageRoot = document.getElementById('mf-pipeline-files-page');
  ...
  Promise.all([MFPrefs.load(), fetchMe()]).then(function (results) {
    ...
    /* Members should not land on the operator pipeline-files page. */
    if (me.role === 'member') {
    ...
    var layoutPop = MFLayoutPopover.create({
      ...
      onChoose: function (mode) {
        ...
        MFTelemetry.emit('ui.layout_mode_selected', { mode: mode, source: 'pipeline_files_popover' });
      },
    });
    ...
    MFPipelineFiles.mount(pageRoot, { role: me.role });

  }).catch(function (e) {
    console.error('mf: pipeline-files boot failed', e);
    var msg = document.createElement('div');
    msg.style.cssText = 'padding:2rem;text-align:center;color:#888;font-family:sans-serif';
    msg.textContent = 'Pipeline Files unavailable. Check console.';
