/* Boot script for the new-UX Flagged Files page (flagged-new.html).
 ...
 *   - Paginated table with per-row actions (dismiss, extend, remove, un-blocklist)
 *   - Auto-refresh every 30 s while page is visible
 *
...
(function () {
  ...
  var pageRoot = document.getElementById('mf-flagged-page');
  ...
  Promise.all([MFPrefs.load(), fetchMe()]).then(function (results) {
    ...
    var layoutPop = MFLayoutPopover.create({
      ...
      onChoose: function (mode) {
        ...
        MFTelemetry.emit('ui.layout_mode_selected', { mode: mode, source: 'flagged_popover' });
      },
    });

    /* Top nav — null activePage: Flagged is not a primary nav item */
    ...
    MFFlagged.mount(pageRoot, { role: me.role });

  }).catch(function (e) {
    console.error('mf: flagged-new boot failed', e);
    var msg = document.createElement('div');
    msg.style.cssText = 'padding:2rem;text-align:center;color:#888;font-family:sans-serif';
    msg.textContent = 'Flagged Files unavailable. Check console.';
