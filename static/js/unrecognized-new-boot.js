/* Boot script for the new-UX Unrecognized Files page.
 * Fetches /api/me, mounts chrome + MFUnrecognized.
...
(function () {
  ...
  var pageRoot = document.getElementById('mf-unrecognized-page');
  ...
  Promise.all([MFPrefs.load(), fetchMe()]).then(function (results) {
    ...
    var layoutPop = MFLayoutPopover.create({
      ...
      onChoose: function (mode) {
        ...
        MFTelemetry.emit('ui.layout_mode_selected', { mode: mode, source: 'unrecognized_popover' });
      },
    });
    ...
    MFUnrecognized.mount(pageRoot, { role: me.role });

  }).catch(function (e) {
    console.error('mf: unrecognized boot failed', e);
    var msg = document.createElement('div');
    msg.style.cssText = 'padding:2rem;text-align:center;color:#888;font-family:sans-serif';
    msg.textContent = 'Unrecognized Files unavailable. Check console.';
