/* Boot script for the new-UX Convert page.
 * Fetches /api/me, mounts chrome + MFConvert.
 * All roles can access the convert page.
 *
 * Safe DOM throughout. */
(function () {
  'use strict';
  ...
  var convertRoot = document.getElementById('mf-convert-page');
  ...
  Promise.all([MFPrefs.load(), fetchMe()]).then(function (results) {
    ...
    MFTopNav.mount(navRoot, { role: me.role, activePage: 'convert' });
    MFVersionChip.mount(
    ...
    MFConvert.mount(convertRoot, { role: me.role });

  }).catch(function (e) {
    console.error('mf: convert boot failed', e);
    var msg = document.createElement('div');
    msg.style.cssText = 'padding:2rem;text-align:center;color:#888;font-family:sans-serif';
    msg.textContent = 'Convert page unavailable. Check console.';
    if (convertRoot) convertRoot.appendChild(msg);
  });
})();
