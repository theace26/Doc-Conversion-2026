/* Boot script for the new-UX Trash page.
 * Fetches /api/me, mounts chrome + MFTrash.
...
(function () {
  ...
  var pageRoot = document.getElementById('mf-trash-page');
  ...
  Promise.all([MFPrefs.load(), fetchMe()]).then(function (results) {
    ...
    var layoutPop = MFLayoutPopover.create({
      ...
      onChoose: function (mode) {
        ...
        MFTelemetry.emit('ui.layout_mode_selected', { mode: mode, source: 'trash_popover' });
      },
    });
    ...
    MFTrash.mount(pageRoot, { role: me.role });

  }).catch(function (e) {
    console.error('mf: trash boot failed', e);
    var msg = document.createElement('div');
    msg.style.cssText = 'padding:2rem;text-align:center;color:#888;font-family:sans-serif';
    msg.textContent = 'Trash unavailable. Check console.';
