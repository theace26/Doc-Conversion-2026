/* Boot script for the new-UX Log Viewer page.
 * Mounts new-UX chrome then hands off to MFLogViewer.mount().
...
(function () {
  ...
  var viewerRoot = document.getElementById('mf-log-viewer');
  ...
  Promise.all([MFPrefs.load(), fetchMe()]).then(function (results) {
    ...
    MFLogViewer.mount(viewerRoot);

  }).catch(function (e) {
    console.error('mf: log-viewer-new boot failed', e);
    if (viewerRoot) {
      var msg = document.createElement('div');
      msg.style.cssText = 'padding:2rem;text-align:center;color:#888;font-family:sans-serif';
      msg.textContent = 'Log viewer unavailable. Check console.';
      viewerRoot.appendChild(msg);
    }
  });
})();
