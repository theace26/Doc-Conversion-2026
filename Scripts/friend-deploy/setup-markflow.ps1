...
# ===========================================================================
Write-Host ""
Write-Host "[3/7] Detecting hardware for optimal configuration..." -ForegroundColor Yellow

# Auto-detect CPU threads and RAM
$detectedCores = 0
$detectedRamGB = 0

try {
    $cpuInfo = Get-CimInstance Win32_Processor -ErrorAction SilentlyContinue
    $detectedCores = ($cpuInfo | Measure-Object -Property NumberOfLogicalProcessors -Sum).Sum
    $ramBytes = (Get-CimInstance Win32_ComputerSystem -ErrorAction SilentlyContinue).TotalPhysicalMemory
    $detectedRamGB = [math]::Floor($ramBytes / 1GB)
}
catch { }

Write-Host ""
if ($detectedCores -gt 0 -and $detectedRamGB -gt 0) {
    Write-Host "  Detected: $detectedCores CPU threads, $detectedRamGB GB RAM" -ForegroundColor Green
    Write-Host ""
    Write-Host "  MarkFlow uses these values to set parallel worker count and" -ForegroundColor DarkGray
    Write-Host "  Meilisearch search engine memory. You can override if needed." -ForegroundColor DarkGray
    Write-Host ""
    $userCoresInput = Read-Host "  CPU threads to use [$detectedCores]"
    if ([string]::IsNullOrWhiteSpace($userCoresInput)) { $userCores = $detectedCores } else { $userCores = [int]$userCoresInput }
    $userRamInput = Read-Host "  Total RAM in GB [$detectedRamGB]"
    if ([string]::IsNullOrWhiteSpace($userRamInput)) { $userRamGB = $detectedRamGB } else { $userRamGB = [int]$userRamInput }
}
else {
    Write-Host "  Could not auto-detect hardware. Please enter your specs:" -ForegroundColor Yellow
    Write-Host "  (Check Task Manager > Performance for CPU threads and RAM)" -ForegroundColor DarkGray
    Write-Host ""
    $userCores = [int](Read-Host "  CPU threads (logical cores)")
    $userRamGB = [int](Read-Host "  Total RAM in GB")
}

# Calculate optimal worker count: ~67% of threads, min 2, max 12
$calcWorkers = [math]::Floor($userCores * 2 / 3)
if ($calcWorkers -lt 2) { $calcWorkers = 2 }
if ($calcWorkers -gt 12) { $calcWorkers = 12 }

# Calculate Meilisearch memory based on RAM
if ($userRamGB -ge 32) {
    $meiliMem = "1g"
    $meiliMemBytes = 1073741824
}
elseif ($userRamGB -ge 16) {
    $meiliMem = "512m"
    $meiliMemBytes = 536870912
}
elseif ($userRamGB -ge 8) {
    $meiliMem = "256m"
    $meiliMemBytes = 268435456
}
else {
    $meiliMem = "128m"
    $meiliMemBytes = 134217728
}

# Generate Meilisearch master key
$meiliKeyBytes = New-Object byte[] 32
[System.Security.Cryptography.RandomNumberGenerator]::Fill($meiliKeyBytes)
$meiliKey = ($meiliKeyBytes | ForEach-Object { $_.ToString("x2") }) -join ""

Write-Host ""
Write-Host "  [OK] Workers: $calcWorkers (of $userCores threads)" -ForegroundColor Green
Write-Host "  [OK] Meilisearch memory: $meiliMem (of ${userRamGB}GB RAM)" -ForegroundColor Green
Write-Host "  [OK] Meilisearch API key generated" -ForegroundColor Green

# ===========================================================================
#  Detect available drive letters for the directory browser
# ===========================================================================
Write-Host ""
Write-Host "[4/7] Detecting drives and GPU..." -ForegroundColor Yellow
...
# ===========================================================================
Write-Host ""
Write-Host "[5/7] Writing .env configuration..." -ForegroundColor Yellow
...
$envLines = @(
    ...
    "# Hardware: $userCores threads, ${userRamGB}GB RAM"
    ""
...
$envLines += @(
    ...
    "MEILI_MEMORY_LIMIT=$meiliMem"
    "MEILI_MAX_INDEXING_MEMORY=$meiliMemBytes"
    ""
    "# Bulk conversion (tuned for $userCores threads)"
    "BULK_WORKER_COUNT=$calcWorkers"
    ""
    "# App"
...
Write-Host "    WORKERS       = $calcWorkers"
Write-Host "    MEILI_MEMORY  = $meiliMem"
...
Write-Host ""
Write-Host "[6/7] Preparing Docker environment..." -ForegroundColor Yellow
...
# ===========================================================================
Write-Host ""
Write-Host "[7/7] Building and starting MarkFlow..." -ForegroundColor Yellow
