...
- **MCP server binding**: `FastMCP.run()` does NOT accept `host` or `port` kwargs. Set `os.environ["UVICORN_HOST"] = "0.0.0.0"` and `os.environ["UVICORN_PORT"]` before calling `mcp.run(transport="sse")`. Without this, Uvicorn defaults to 127.0.0.1:8000 which is unreachable from outside the Docker container.
