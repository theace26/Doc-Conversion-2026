...
- **Scan priority coordinator**: `core/scan_coordinator.py` enforces Bulk > Run Now > Lifecycle. Bulk starting cancels lifecycle (clean cancel, releases DB) and pauses run-now (auto-resumes when bulk completes). Run-now cancels lifecycle. Lifecycle never pauses — only cancels and picks up at next scheduled interval. Cancelled lifecycle scans skip deletion detection and auto-conversion trigger. On startup, `reset_coordinator()` clears all in-memory flags to prevent ghost state from a previous container lifecycle. A watchdog (`check_stale_scans()`, every 5 min) auto-resets scans stuck "running" beyond 4 hours.
- **Lifecycle scanner flushes counters every 500 files**: `_flush_counters_to_db()` persists scan_run counters periodically during both serial and parallel walks, so crash recovery preserves progress instead of showing all-zeros.
